...
const isOdd = (num) => {
  if (typeof num === 'string') {
    ...
    return false
  } else if (num.toString() !== 'NaN' && typeof num === 'number') {
  return (num % 2 !== 0)
  } else {
  return false
  }
...
const secondSmallest = (array) => {
  let smallest = (array.reduce((acc,currEl) => {
    if (acc < currEl) {
      return acc
    } else {
      return currEl
  ...
  let noSmallest = array.filter (a => a !== smallest)
  return (noSmallest.reduce((acc,currEl) => {
    if (acc < currEl) {
      return acc
    } else {
      return currEl
...
const getLocations = (array) => {
  return array.map(obj => obj = obj['location'])
}
